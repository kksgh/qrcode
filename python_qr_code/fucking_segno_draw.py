...
segno.make('https://lk.profi-time.com/', error='h').save(out, scale=5, kind='png')
out.seek(0)  # Important to let Pillow load the PNG
img = Image.open(out)
img = img.convert('RGB')  # Ensure colors for the output
img_width, img_height = img.size
logo_max_size = img_height // 4  # May use a fixed value as well
logo_img = Image.open('backs/teleg.png')  # The logo
